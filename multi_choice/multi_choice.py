...
class DataTrainingArguments:
    ...
    context_file: Optional[str] = field(
        default=None,
        metadata={"help": "An optional input context data file to train."},
    )
...
def main():
    ...
    logger.info("Finishing loading files...")
    ...
    # paragraph_name = "paragraphs"
    answer_name = "relevant"
    ...
    def preprocess_function(examples):
        ...
        second_sentences = [
            [context_json[idx] for idx in idxs] for idxs in question_headers
        ]
        ...
        if("answer" in examples.keys()):
            data["label"] = [examples['paragraphs'][i].index(ans) for i,ans in enumerate(examples['relevant'])]
        return data
    ...
    if training_args.do_train:
        ...
        # trainer.model.save_pretrained(model_args.model_name_or_path+"/model")
        # trainer.tokenizer.save_pretrained(model_args.model_name_or_path+"/tokenizer")
    ...
    if(training_args.do_predict):
        # tokenizer = AutoTokenizer.from_pretrained(
        #     # model_args.tokenizer_name if model_args.tokenizer_name else model_args.model_name_or_path,
        #     model_args.model_name_or_path+"/tokenizer",
        #     cache_dir=model_args.cache_dir,
        #     use_fast=model_args.use_fast_tokenizer,
        #     revision=model_args.model_revision,
        #     use_auth_token=True if model_args.use_auth_token else None,
        ...
        for i,p in enumerate(predictions):
            ...
            output.append({
                ...
                "relevant" : test_dataset["paragraphs"][i][p]
            })
